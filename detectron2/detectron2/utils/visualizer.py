...
class Visualizer:
    ...
    def draw_instance_predictions_custom(self, predictions, 
        incl_boxes=True, 
        incl_labels=True, 
        incl_scores=True, 
        target_alpha=None):
        ...
        if self._instance_mode == ColorMode.SEGMENTATION and self.metadata.get("thing_colors"):
            ...
            for c in classes:
                ind += 1
                color = self._jitter([x / 255 for x in self.metadata.thing_colors[ind]])
                colors.append(color)

            alpha = 0.8
        elif self.metadata.get("thing_colors"):
            colors = []
            ind = -1
            for c in classes:
                ind += 1
                color = [x / 255 for x in self.metadata.thing_colors[ind]]
                colors.append(color)
    ...
    def draw_panoptic_seg_predictions_custom(
        self, panoptic_seg, segments_info, area_threshold=None, alpha=0.7,
        incl_labels=True, incl_scores=True
    ):
        pred = _PanopticPrediction(panoptic_seg, segments_info)
        ...
        for mask, sinfo in pred.semantic_masks():
            ...
            try:
                # mask_color = [x / 255 for x in self.metadata.stuff_colors[category_idx]]
                mask_color = [x / 255 for x in self.metadata.stuff_colors[ind]]
            except AttributeError:
                mask_color = None

            text = self.metadata.stuff_classes[category_idx] if incl_labels else None
            self.draw_binary_mask(
                mask,
                color=mask_color,
                edge_color=_OFF_WHITE,
        ...
        try:
            # colors = [random_color(rgb=True, maximum=1) for k in category_ids]
            colors = []
            for ind, _ in enumerate(category_ids):
                mask_color = [x / 255 for x in self.metadata.thing_colors[ind]]
